---
description: Update Penguin from upstream when branch has diverged (ahead/behind)
---

# Penguin Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "Penguin" || true

# Move old version
mv /Applications/Penguin.app /tmp/Penguin-backup.app

# Install new build
cp -R dist/Penguin.app /Applications/
...
ps aux | grep -E "(penguin|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm penguin agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update penguin.json with fallback chains
...
pnpm penguin agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
