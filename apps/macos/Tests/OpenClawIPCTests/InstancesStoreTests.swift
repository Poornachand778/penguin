...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: PenguinProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: PenguinProtocol.AnyCodable] = [
            "presence": .init([PenguinProtocol.AnyCodable(entry)]),
        ]
        let payload = PenguinProtocol.AnyCodable(payloadMap)
