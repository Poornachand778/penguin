...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.penguin", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: PenguinPeekabooBridgeServices?
    private static var penguinSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Penguin", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = PenguinPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.penguinSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.penguinSocketPath, privacy: .public)")
    }
...
@MainActor
private final class PenguinPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.penguin.peekaboo")
