...
private let webChatSwiftLogger = Logger(subsystem: "ai.penguin", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: PenguinChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> PenguinChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(PenguinChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> PenguinChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                PenguinGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<PenguinChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any PenguinChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = PenguinChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: PenguinChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<PenguinChatView>) -> NSViewController
    {
