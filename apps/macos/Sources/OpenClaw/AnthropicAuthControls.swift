...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: PenguinOAuthStore.AnthropicOAuthStatus = PenguinOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: PenguinOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = PenguinOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = PenguinOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try PenguinOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Penguin can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: PenguinOAuthStore.AnthropicOAuthStatus,
