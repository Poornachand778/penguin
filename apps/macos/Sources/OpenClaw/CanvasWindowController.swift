...
final class CanvasWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
    ...
    init(sessionKey: String, root: URL, presentation: CanvasPresentation) throws {
        ...
        (() => {
          try {
            ...
            if (globalThis.__penguinA2UIBridgeInstalled) return;
            globalThis.__penguinA2UIBridgeInstalled = true;
            ...
            globalThis.addEventListener('a2uiaction', (evt) => {
              try {
                ...
                const handler = globalThis.webkit?.messageHandlers?.penguinCanvasA2UIAction;
                ...
                const hasBundledA2UIHost =
                  !!globalThis.penguinA2UI ||
                  !!document.querySelector('penguin-a2ui-host');
                ...
                location.href = 'penguin://agent?' + params.toString();
              } catch {}
            }, true);
          } catch {}
        })();
    ...
    func applyDebugStatusIfNeeded() {
        ...
        (() => {
          try {
            const api = globalThis.__penguin;
            if (!api) return;
    ...
    func snapshot(to outPath: String?) async throws -> String {
        ...
        if let outPath, !outPath.isEmpty {
            ...
            path = "/tmp/penguin-canvas-\(CanvasWindowController.sanitizeSessionKey(self.sessionKey))-\(ts).png"
        }
