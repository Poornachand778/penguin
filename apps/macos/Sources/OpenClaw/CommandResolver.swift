...
enum CommandResolver {
    private static let projectRootDefaultsKey = "penguin.gatewayProjectRootPath"
    private static let helperName = "penguin"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let penguinEntry = root.appendingPathComponent("penguin.mjs").path
        if FileManager().isReadableFile(atPath: penguinEntry) { return penguinEntry }
        let binEntry = root.appendingPathComponent("bin/penguin.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/penguin")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let penguinPaths = self.penguinManagedPaths(home: home)
        if !penguinPaths.isEmpty {
            extras.insert(contentsOf: penguinPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + penguinPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func penguinManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".penguin"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func penguinExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectPenguinExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("penguin.mjs").path,
            root.appendingPathComponent("bin/penguin.js").path,
        ]
    ...
    static func hasAnyPenguinInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.penguinExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func penguinNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let penguinPath = self.projectPenguinExecutable(projectRoot: root) {
                return [penguinPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "penguin", subcommand] + extraArgs
            }
            if let penguinPath = self.penguinExecutable(searchPaths: searchPaths) {
                return [penguinPath, subcommand] + extraArgs
            }

            let missingEntry = """
            penguin entrypoint missing (looked for dist/index.js or penguin.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real penguin CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/penguin"
        ...
        \(cliSection)
        \(projectSection)
        if command -v penguin >/dev/null 2>&1; then
          CLI="$(command -v penguin)"
          penguin \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/penguin.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/penguin.mjs"
            node "$PRJ/penguin.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/penguin.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/penguin.js"
            node "$PRJ/bin/penguin.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent penguin"
          pnpm --silent penguin \(quotedArgs);
        else
          echo "penguin CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? PenguinConfigFile.loadDict()
