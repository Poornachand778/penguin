...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.penguin", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> PenguinSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: PenguinSessionPreviewEntry,
    ...
    private static func cache(payload: PenguinSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: PenguinChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [PenguinKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [PenguinKit.AnyCodable]) -> [PenguinChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(PenguinChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: PenguinChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: PenguinChatMessage) -> Bool {
    ...
    private static func toolNames(for message: PenguinChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: PenguinChatMessage) -> String? {
