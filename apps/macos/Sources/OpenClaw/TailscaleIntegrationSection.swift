...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.penguin/penguin.json. Prefer PENGUIN_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.penguin/penguin.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.penguin/penguin.json. Restart the gateway to apply."
        }
