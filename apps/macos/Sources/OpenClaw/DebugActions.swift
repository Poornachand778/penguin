...
enum DebugActions {
    private static let verboseDefaultsKey = "penguin.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "penguin.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = PenguinPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Penguin", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = PenguinPaths.configURL
        guard
