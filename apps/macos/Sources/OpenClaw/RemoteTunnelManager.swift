...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.penguin", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Penguin run already has an SSH listener on the expected port (common after restarts),
