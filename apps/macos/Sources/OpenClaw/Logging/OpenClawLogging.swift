...
enum PenguinLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = PenguinOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = PenguinFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        PenguinLogging.bootstrapIfNeeded()
        let label = PenguinLogging.makeLabel(subsystem: subsystem, category: category)
...
struct PenguinOSLogHandler: LogHandler {
...
struct PenguinFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = PenguinLogging.parseLabel(self.label)
