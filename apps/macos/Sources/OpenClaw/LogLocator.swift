...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["PENGUIN_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/penguin")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("penguin-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("penguin-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/penguin/ (rolling or stdout), or nil if none exist.
