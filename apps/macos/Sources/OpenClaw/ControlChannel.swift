...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: PenguinProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.penguin", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: PenguinKit.AnyCodable]()) {
                $0[$1.key] = PenguinKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: PenguinProtocol.AnyCodable?) -> [String: PenguinProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: PenguinProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: PenguinKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: PenguinProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("penguin.control.heartbeat")
    static let controlAgentEvent = Notification.Name("penguin.control.agent")
}
