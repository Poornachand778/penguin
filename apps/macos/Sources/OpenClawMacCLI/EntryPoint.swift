...
struct PenguinMacCLI {
    static func main() async {
        ...
        default:
            fputs("penguin-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      penguin-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      penguin-mac discover [--timeout <ms>] [--json] [--include-local]
      penguin-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      penguin-mac connect
      penguin-mac connect --url ws://127.0.0.1:18789 --json
      penguin-mac discover --timeout 3000 --json
      penguin-mac wizard --mode local
    """)
}
