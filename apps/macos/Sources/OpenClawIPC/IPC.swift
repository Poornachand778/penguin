...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/Penguin/canvas/<session>/`).
...
public let controlSocketPath: String = {
    ...
    let preferred = home
        .appendingPathComponent("Library/Application Support/Penguin/control.sock")
        .path
    return preferred
}()
