...
struct IOSGatewayChatTransport: PenguinChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> PenguinChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(PenguinChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> PenguinChatHistoryPayload {
        ...
        return try JSONDecoder().decode(PenguinChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [PenguinChatAttachmentPayload]) async throws -> PenguinChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [PenguinChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(PenguinChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(PenguinGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PenguinGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PenguinChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PenguinAgentEventPayload.self)
                        {
