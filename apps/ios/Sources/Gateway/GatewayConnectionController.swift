...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [PenguinCapability.canvas.rawValue, PenguinCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(PenguinCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(PenguinCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = PenguinLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(PenguinCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            PenguinSystemCommand.notify.rawValue,
            PenguinSystemCommand.which.rawValue,
            PenguinSystemCommand.run.rawValue,
            PenguinSystemCommand.execApprovalsGet.rawValue,
            PenguinSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(PenguinCapability.camera.rawValue) {
            commands.append(PenguinCameraCommand.list.rawValue)
            commands.append(PenguinCameraCommand.snap.rawValue)
            commands.append(PenguinCameraCommand.clip.rawValue)
        }
        if caps.contains(PenguinCapability.location.rawValue) {
            commands.append(PenguinLocationCommand.get.rawValue)
        }
