...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = PenguinCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = PenguinCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = PenguinCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = PenguinCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = PenguinCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__penguin__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: PenguinLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for penguin:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case PenguinLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case PenguinCanvasCommand.present.rawValue,
                 PenguinCanvasCommand.hide.rawValue,
                 PenguinCanvasCommand.navigate.rawValue,
                 PenguinCanvasCommand.evalJS.rawValue,
                 PenguinCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case PenguinCanvasA2UICommand.reset.rawValue,
                 PenguinCanvasA2UICommand.push.rawValue,
                 PenguinCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case PenguinCameraCommand.list.rawValue,
                 PenguinCameraCommand.snap.rawValue,
                 PenguinCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case PenguinScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: PenguinNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: PenguinNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(PenguinLocationGetParams.self, from: req.paramsJSON)) ??
            PenguinLocationGetParams()
        ...
        let payload = PenguinLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PenguinCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(PenguinCanvasPresentParams.self, from: req.paramsJSON)) ??
                PenguinCanvasPresentParams()
        ...
        case PenguinCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case PenguinCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(PenguinCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case PenguinCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(PenguinCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PenguinCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(PenguinCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case PenguinCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing penguinA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PenguinCanvasA2UICommand.push.rawValue, PenguinCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == PenguinCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(PenguinCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try PenguinCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(PenguinCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try PenguinCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.penguinA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case PenguinCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PenguinCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(PenguinCameraSnapParams.self, from: req.paramsJSON)) ??
                PenguinCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PenguinCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(PenguinCameraClipParams.self, from: req.paramsJSON)) ??
                PenguinCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(PenguinScreenRecordParams.self, from: req.paramsJSON)) ??
            PenguinScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> PenguinLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return PenguinLocationMode(rawValue: raw) ?? .off
    }
