...
import "@a2ui/lit/ui";
import { themeContext } from "@penguin/a2ui-theme-context";
...
class PenguinA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--penguin-a2ui-inset-top, 0px)
        var(--penguin-a2ui-inset-right, 0px)
        var(--penguin-a2ui-inset-bottom, 0px)
        var(--penguin-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--penguin-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--penguin-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--penguin-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--penguin-a2ui-empty-top, var(--penguin-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.penguinA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["penguin:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__penguinLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.penguinCanvasA2UIAction ??
      globalThis.penguinCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.penguinCanvasA2UIAction) {
...
if (!customElements.get("penguin-a2ui-host")) {
  customElements.define("penguin-a2ui-host", PenguinA2UIHost);
}
