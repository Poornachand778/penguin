...
struct ChatMessageBubble: View {
    let message: PenguinChatMessage
    let style: PenguinChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? PenguinChatTheme.userText : PenguinChatTheme.assistantText
    ...
    private var inlineAttachments: [PenguinChatMessageContent] {
    ...
    private var toolCalls: [PenguinChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [PenguinChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? PenguinChatTheme.userBubble
        }
        if self.style == .onboarding {
            return PenguinChatTheme.onboardingAssistantBubble
        }
        return PenguinChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return PenguinChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: PenguinChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? PenguinChatTheme.userText : PenguinChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: PenguinChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(PenguinChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(PenguinChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [PenguinChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: PenguinChatTheme.assistantText)
            }
        }
    }
}
