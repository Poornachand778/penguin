...
private final class TestChatTransport: @unchecked Sendable, PenguinChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [PenguinChatHistoryPayload]
    private let sessionsResponses: [PenguinChatSessionsListResponse]

    private let stream: AsyncStream<PenguinChatTransportEvent>
    private let continuation: AsyncStream<PenguinChatTransportEvent>.Continuation

    init(
        historyResponses: [PenguinChatHistoryPayload],
        sessionsResponses: [PenguinChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<PenguinChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> PenguinChatHistoryPayload {
        ...
        return self.historyResponses.last ?? PenguinChatHistoryPayload(
        ...
        attachments _: [PenguinChatAttachmentPayload]) async throws -> PenguinChatSendResponse
    {
        ...
        return PenguinChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> PenguinChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? PenguinChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: PenguinChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = PenguinChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = PenguinChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { PenguinChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = PenguinChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = PenguinChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = PenguinChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { PenguinChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
