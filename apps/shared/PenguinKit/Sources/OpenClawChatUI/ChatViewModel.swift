...
private let chatUILogger = Logger(subsystem: "ai.penguin", category: "PenguinChatUI")

@MainActor
@Observable
public final class PenguinChatViewModel {
    public private(set) var messages: [PenguinChatMessage] = []
    ...
    public var attachments: [PenguinPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [PenguinChatPendingToolCall] = []
    public private(set) var sessions: [PenguinChatSessionEntry] = []
    private let transport: any PenguinChatTransport
    ...
    private var pendingToolCallsById: [String: PenguinChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any PenguinChatTransport) {
    ...
    public var sessionChoices: [PenguinChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [PenguinChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [PenguinChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: PenguinPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [PenguinChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: PenguinChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [PenguinChatMessage]) -> [PenguinChatMessage] {
        var result: [PenguinChatMessage] = []
    ...
    private static func dedupeKey(for message: PenguinChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [PenguinChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> PenguinChatAttachmentPayload in
            PenguinChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> PenguinChatSessionEntry {
        PenguinChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: PenguinChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: PenguinChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: PenguinAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = PenguinChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> PenguinPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
