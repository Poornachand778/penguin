...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/penguin/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/penguin");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/penguin/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__PENGUIN_CONTROL_UI_BASE_PATH__ = "/penguin";
    const app = mountApp("/penguin/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/penguin");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/penguin/sessions");
  });
