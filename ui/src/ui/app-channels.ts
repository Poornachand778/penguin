...
export async function handleWhatsAppStart(host: PenguinApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: PenguinApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: PenguinApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: PenguinApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: PenguinApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: PenguinApp): string {
...
export function handleNostrProfileCancel(host: PenguinApp) {
...
export function handleNostrProfileToggleAdvanced(host: PenguinApp) {
  const state = host.nostrProfileFormState;
  if (!state) return;
  host.nostrProfileFormState = {
    ...state,
...
export async function handleNostrProfileSave(host: PenguinApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: PenguinApp) {
  const state = host.nostrProfileFormState;
