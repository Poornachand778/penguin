...
function trySetupCompletion(repoRoot) {
  ...
  if (process.env.CI || process.env.PENGUIN_SKIP_COMPLETION_SETUP) {
    return;
  }

  const binPath = path.join(repoRoot, "penguin.mjs");
  ...
  try {
    // Run with PENGUIN_SKIP_POSTINSTALL to avoid any weird recursion,
    // though distinct from this script.
    spawnSync(process.execPath, [binPath, "completion", "--install", "--yes"], {
      cwd: repoRoot,
      stdio: "inherit",
      env: { ...process.env, PENGUIN_SKIP_POSTINSTALL: "1" },
    });
  } catch {
