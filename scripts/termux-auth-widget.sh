...
# Penguin Auth Widget for Termux
...
SERVER="${PENGUIN_SERVER:-${PENGUIN_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Penguin auth..."

STATUS=$(ssh "$SERVER" '$HOME/penguin/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/penguin/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|PENGUIN_EXPIRING|PENGUIN_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/penguin/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|PENGUIN_EXPIRED|PENGUIN_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Penguin won't work until re-auth"
                ;;
        esac
        ;;
