...
IMAGE_NAME="penguin-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="penguin-net-e2e-$$"
GW_NAME="penguin-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "PENGUIN_GATEWAY_TOKEN=$TOKEN" \
	  -e "PENGUIN_SKIP_CHANNELS=1" \
	  -e "PENGUIN_SKIP_GMAIL_WATCHER=1" \
	  -e "PENGUIN_SKIP_CRON=1" \
	  -e "PENGUIN_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
