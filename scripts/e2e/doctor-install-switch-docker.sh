...
IMAGE_NAME="penguin-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/penguin-bin:$PATH"
  mkdir -p /tmp/penguin-bin

  cat > /tmp/penguin-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/penguin-bin/systemctl

  cat > /tmp/penguin-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/penguin-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/penguin"
  npm_entry="/tmp/npm-prefix/lib/node_modules/penguin/penguin.mjs"
  git_entry="/app/penguin.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/penguin-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/penguin-gateway.service"
    if [ ! -f "$unit_path" ]; then
