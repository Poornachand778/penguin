...
IMAGE_NAME="penguin-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/penguin-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/penguin-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/penguin-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/penguin-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/penguin"
    config_path="$HOME/.penguin/penguin.json"
    sessions_dir="$HOME/.penguin/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.penguin/penguin.json" <<'"'"'JSON'"'"'
{
