...
const compilerOverride = env.PENGUIN_TS_COMPILER ?? env.PENGUIN_TS_COMPILER;
...
const logRunner = (message) => {
  if (env.PENGUIN_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[penguin] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["penguin.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
