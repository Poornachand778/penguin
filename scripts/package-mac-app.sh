...
# Build and bundle Penguin into a minimal .app we can open.
# Outputs to dist/Penguin.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Penguin.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Penguin"
BUNDLE_ID="${BUNDLE_ID:-ai.penguin.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/penguin/penguin/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Penguin/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :PenguinBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :PenguinGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Penguin"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Penguin"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Penguin"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Penguin" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Penguin/Resources/Penguin.icns" "$APP_ROOT/Contents/Resources/Penguin.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Penguin/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying PenguinKit resources"
PENGUINKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/PenguinKit_PenguinKit.bundle"
if [ -d "$PENGUINKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/PenguinKit_PenguinKit.bundle"
  cp -R "$PENGUINKIT_BUNDLE" "$APP_ROOT/Contents/Resources/PenguinKit_PenguinKit.bundle"
else
  echo "WARN: PenguinKit resource bundle not found at $PENGUINKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Penguin"
killall -q Penguin 2>/dev/null || true
