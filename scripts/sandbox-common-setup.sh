...
BASE_IMAGE="${BASE_IMAGE:-penguin-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-penguin-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=penguin.sandbox=1)
NOTE
