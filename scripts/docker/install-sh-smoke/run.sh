...
INSTALL_URL="${PENGUIN_INSTALL_URL:-https://penguin.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${PENGUIN_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${PENGUIN_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="penguin"
PACKAGE_NAME="${PENGUIN_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (PENGUIN_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${PENGUIN_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${PENGUIN_INSTALL_LATEST_OUT:-}"
fi
