...
#
# Suggested cron: */30 * * * * /home/admin/penguin/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Penguin notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., penguin-alerts)
...
STATE_FILE="$HOME/.penguin/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Penguin if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use penguin
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Penguin to $NOTIFY_PHONE..."
            penguin send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Penguin is down. Run: ssh l36 '~/penguin/scripts/mobile-reauth.sh'" "urgent"
    exit 1
