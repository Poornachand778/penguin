#!/bin/bash
# Setup Penguin Auth Management System
...
echo "=== Penguin Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'penguin-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Penguin message: Send warning via Penguin itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/penguin-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/penguin-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/penguin-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now penguin-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status penguin-auth-monitor.timer"
echo ""
echo "Quick commands:"
