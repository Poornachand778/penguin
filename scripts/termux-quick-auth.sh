...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/penguin/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Penguin Re-Auth" -c "After getting key, run: ssh $SERVER '~/penguin/scripts/mobile-reauth.sh'" --id penguin-auth
        ;;
    *)
