...
ZIP=${1:?"Usage: $0 Penguin-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/penguin/penguin/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Penguin-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/penguin/penguin/releases/download/v${VERSION}/"}
