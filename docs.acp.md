...
This document describes how the Penguin ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`penguin acp` exposes an ACP agent over stdio and forwards prompts to a running
Penguin Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Penguin Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `penguin acp` over stdio.

Example config:

```bash
penguin config set gateway.remote.url wss://gateway-host:18789
penguin config set gateway.remote.token <token>
```

Example run:

```bash
penguin acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
penguin acp --session agent:main:main
penguin acp --session agent:design:main
penguin acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Penguin ACP” to start a thread.

## Execution Model

- ACP client spawns `penguin acp` and speaks ACP messages over stdio.
...
```bash
penguin acp --session agent:main:main
penguin acp --session-label "support inbox"
penguin acp --reset-session
```
...
`penguin acp` resolves the Gateway URL and auth from CLI flags or config:
