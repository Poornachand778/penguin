...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `penguin-e2e-${name}-`));
  const configDir = path.join(homeDir, ".penguin");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          PENGUIN_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        PENGUIN_GATEWAY_TOKEN: inst.gatewayToken,
        PENGUIN_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          PENGUIN_GATEWAY_PORT: String(gwA.port),
          PENGUIN_GATEWAY_TOKEN: gwA.gatewayToken,
          PENGUIN_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          PENGUIN_GATEWAY_PORT: String(gwB.port),
          PENGUIN_GATEWAY_TOKEN: gwB.gatewayToken,
          PENGUIN_GATEWAY_PASSWORD: "",
        }),
