...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("PENGUIN_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.PENGUIN_STATE_DIR = path.join(base, ".penguin");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "penguin-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".penguin", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
