...
import type { PenguinConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("penguin-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("penguin-bin-sherpa-");
      const modelDir = await makeTempDir("penguin-sherpa-model-");
      ...
      const cfg: PenguinConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("penguin-bin-whispercpp-");
      const modelDir = await makeTempDir("penguin-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("penguin-bin-gemini-");
      ...
      const cfg: PenguinConfig = { tools: { media: { image: {} } } };
