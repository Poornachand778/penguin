...
- Publish as `penguin/plugin-sdk` (or export from core under `penguin/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `PenguinPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: PenguinConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: PenguinConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: PenguinConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: PenguinConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: PenguinConfig): string;
  };
};
```
...
- Introduce `penguin/plugin-sdk`.
- Add `api.runtime` to `PenguinPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `penguinRuntime: ">=2026.2.0"`).
