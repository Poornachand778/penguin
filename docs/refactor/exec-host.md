...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.penguin/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.penguin/node.json` (node id + pairing token).
...
Path: `~/.penguin/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.penguin/exec-approvals.sock",
...
- Unix socket at `~/.penguin/exec-approvals.sock` (0600).
