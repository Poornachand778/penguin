...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `penguin sandbox explain`.
...
Legacy `agent.*` configs are migrated by `penguin doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=penguin-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${PENGUIN_STATE_DIR:-$HOME/.penguin}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
