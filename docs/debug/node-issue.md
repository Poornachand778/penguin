...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Penguin
---
...
Running Penguin via Node with `tsx` fails at startup with:

```
[penguin] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `penguin status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch penguin.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node penguin.mjs status` works on Node 25.
