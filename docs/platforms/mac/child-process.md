...
service via the external `penguin` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`PENGUIN_PROFILE`; legacy `com.penguin.*` is supported).
