---
summary: "Penguin on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Penguin on a Raspberry Pi
  - Running Penguin on ARM devices
  - Building a cheap always-on personal AI
---

# Penguin on Raspberry Pi

## Goal

Run a persistent, always-on Penguin Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Penguin features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled penguin

# Enable if not
sudo systemctl enable penguin

# Start on boot
sudo systemctl start penguin
```
...
```bash
# Check logs
journalctl -u penguin --no-pager -n 100

# Common fix: rebuild
cd ~/penguin  # if using hackable install
npm run build
sudo systemctl restart penguin
```
