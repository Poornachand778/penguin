...
You can also set `PENGUIN_VERBOSE=1` instead of the flag.
...
### `docs.penguin.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.penguin.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.penguin.ai` to the allowlist, then retry.
...
- Check the allowlist: `penguin config get agents.defaults.models`
...
If you can, include the relevant log tail from `penguin logs --follow`.
