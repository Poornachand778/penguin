---
summary: "Chrome extension: let Penguin drive your existing Chrome tab"
read_when:
...
The Penguin Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate penguin-managed Chrome profile.
...
Penguin then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Penguin release (npm package) as static files. There is no separate “build” step.

After upgrading Penguin:

- Re-run `penguin browser extension install` to refresh the installed files under your Penguin state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Penguin ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Penguin to control.
...
- `ON`: attached; Penguin can drive that tab.
...
Debugging: `penguin sandbox explain`
...
`penguin browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `penguin browser extension install` first to copy the extension to a stable location under your Penguin state directory.
...
- **This is not isolated** like the dedicated penguin-managed profile.
