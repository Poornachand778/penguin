...
- **gateway host** → `penguin` process on the gateway machine
...
`~/.penguin/exec-approvals.json` directly.

CLI: `penguin approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
