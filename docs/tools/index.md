---
summary: "Agent tool surface for Penguin (browser, canvas, nodes, message, cron) replacing legacy `penguin-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `penguin-*` skills
---

# Tools (Penguin)

Penguin exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `penguin-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `penguin.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Penguin logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`penguin node run`).
...
- Requires a Brave API key (recommended: `penguin configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Penguin-managed browser.
...
- Quick smoke: `penguin nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `penguin gateway` restart in-place)
