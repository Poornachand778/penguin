---
title: Lobster
summary: "Typed workflow runtime for Penguin with resumable approval gates."
description: Typed workflow runtime for Penguin — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Penguin run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Penguin runs one Lobster tool call and gets a structured result.
...
Penguin launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | penguin.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
penguin.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Penguin tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Penguin Gateway (see the [Lobster repo](https://github.com/penguin/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Penguin keeps core tools enabled. To restrict core
...
→ penguin calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ penguin calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Penguin tools that do.
