...
- Jobs persist under `~/.penguin/cron/` so restarts don’t lose schedules.
...
penguin cron list
penguin cron run <job-id> --force
penguin cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.penguin/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `penguin cron add/edit` or the cron
...
- Job store: `~/.penguin/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.penguin/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
penguin cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
penguin cron edit <jobId> --agent ops
penguin cron edit <jobId> --clear-agent
```
...
```bash
penguin system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`penguin system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `PENGUIN_SKIP_CRON`.
