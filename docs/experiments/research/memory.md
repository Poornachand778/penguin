...
read_when:
  - Designing workspace memory (~/.penguin/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Penguin integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.penguin/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.penguin/workspace` as canonical human-readable memory.
...
```
~/.penguin/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Penguin**, but keep a separable core library.

### Why integrate into Penguin?

- Penguin already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `penguin memory recall "…" --k 25 --since 30d`
  - `penguin memory reflect --since 7d`
...
Pragmatic take for `~/.penguin/workspace`:
