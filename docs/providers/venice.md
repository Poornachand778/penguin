---
summary: "Use Venice AI privacy-focused models in Penguin"
read_when:
  - You want privacy-focused inference in Penguin
...
```bash
penguin onboard --auth-choice venice-api-key
```
...
```bash
penguin chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Penguin shows all available Venice models. Pick based on your needs:
...
```bash
penguin models set venice/claude-opus-45
penguin models set venice/llama-3.3-70b
```

List all available models:

```bash
penguin models list | grep venice
```

## Configure via `penguin configure`

1. Run `penguin configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Penguin automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
penguin chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
penguin chat --model venice/claude-opus-45

# Use uncensored model
penguin chat --model venice/venice-uncensored

# Use vision model with image
penguin chat --model venice/qwen3-vl-235b-a22b

# Use coding model
penguin chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `penguin models list` to see currently available models. Some models may be temporarily offline.
