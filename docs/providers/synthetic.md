---
summary: "Use Synthetic's Anthropic-compatible API in Penguin"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Penguin registers it as the
...
```bash
penguin onboard --auth-choice synthetic-api-key
```
...
Note: Penguin's Anthropic client appends `/v1` to the base URL, so use
