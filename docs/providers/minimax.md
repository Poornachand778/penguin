---
summary: "Use MiniMax M2.1 in Penguin"
read_when:
  - You want MiniMax models in Penguin
...
```bash
penguin plugins enable minimax-portal-auth  # skip if already loaded.
penguin gateway restart  # restart if gateway is already running
penguin onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/penguin/penguin/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `penguin.json`:
...
- Use `penguin models list` and `penguin models set minimax/MiniMax-M2.1` to switch.
...
- Running `penguin configure` and selecting **MiniMax M2.1**, or
