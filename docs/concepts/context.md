...
read_when:
  - You want to understand what “context” means in Penguin
...
“Context” is **everything Penguin sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Penguin-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Penguin builds the system prompt

The system prompt is **Penguin-owned** and rebuilt each run. It includes:
...
By default, Penguin injects a fixed set of workspace files (if present):
