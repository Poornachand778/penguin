...
- `/usage cost` in chats: local cost summary aggregated from Penguin session logs.
- CLI: `penguin status --usage` prints a full per-provider breakdown.
- CLI: `penguin channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
