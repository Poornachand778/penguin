...
Penguin features that can generate provider usage or paid API calls.
...
- `penguin status --usage` and `penguin channels list` show provider **usage windows**
...
Penguin can pick up credentials from:
...
- `penguin status --usage`
- `penguin models status --json`
...
`penguin models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
