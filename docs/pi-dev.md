...
This guide summarizes a sane workflow for working on the pi integration in Penguin.
...
- Trigger the agent directly:
  - `pnpm penguin agent --message "Hello" --thinking low`
...
State lives under the Penguin state directory. Default is `~/.penguin`. If `PENGUIN_STATE_DIR` is set, use that directory instead.

To reset everything:

- `penguin.json` for config
...
- https://docs.penguin.ai/testing
- https://docs.penguin.ai/start/getting-started
