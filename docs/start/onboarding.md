---
summary: "First-run onboarding flow for Penguin (macOS app)"
read_when:
...
- Writes credentials to `~/.penguin/credentials/oauth.json`
...
The app can install the global `penguin` CLI via npm/pnpm so terminal
...
On the first agent run, Penguin bootstraps a workspace (default `~/.penguin/workspace`):
...
```bash
penguin webhooks gmail setup --account you@gmail.com
```
...
- `~/.penguin/credentials/oauth.json`
