...
- During heartbeat runs, Penguin treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `penguin sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Penguin skips the heartbeat run entirely (no model call).
...
```bash
penguin system event --text "Check for urgent follow-ups" --mode now
```
