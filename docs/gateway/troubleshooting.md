---
summary: "Quick troubleshooting guide for common Penguin failures"
read_when:
...
When Penguin misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `penguin status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `penguin status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `penguin status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `penguin gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `penguin channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `penguin gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `penguin logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `penguin status --all` (it redacts tokens). If you paste `penguin status`, consider setting `PENGUIN_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  penguin models auth setup-token --provider anthropic
  ```
...
```bash
penguin models auth paste-token --provider anthropic
penguin models status
```
...
- Preferred: `penguin logs --follow`
- File logs (always): `/tmp/penguin/penguin-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$PENGUIN_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u penguin-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  penguin config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  penguin config set gateway.mode remote
  penguin config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `penguin setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.penguin/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `penguin doctor`
...
- then run `penguin sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `penguin gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `penguin gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `penguin gateway status` and `penguin doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `PENGUIN_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `penguin gateway status` shows a config mismatch**
...
- Fix: rerun `penguin gateway install --force` from the same `--profile` / `PENGUIN_STATE_DIR` you want the service to use.

**If `penguin gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `penguin doctor` to update it (or `penguin gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `PENGUIN_GATEWAY_TOKEN`) and restart the service.

**If `penguin gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/penguin` on disk.
...
Symptoms: `pwd` or file tools show `~/.penguin/sandboxes/...` even though you
...
Penguin intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `penguin models list` or
  `penguin models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${PENGUIN_CONFIG_PATH:-$HOME/.penguin/penguin.json}"
```
...
tail -f "$(ls -t /tmp/penguin/penguin-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
penguin logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.penguin/agents/<agentId>/sessions/
```
...
penguin logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
penguin gateway --verbose
```

If you’re logged out / unlinked:

```bash
penguin channels logout
trash "${PENGUIN_STATE_DIR:-$HOME/.penguin}/credentials" # if logout can't cleanly remove everything
penguin channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/penguin/penguin-*.log | head -1)" | tail -20
```

### High Memory Usage

Penguin keeps conversation history in memory.
...
Penguin now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `penguin doctor` reports every invalid entry.
- `penguin doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `penguin logs`, `penguin health`, `penguin status`, `penguin gateway status`, and `penguin gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/penguin/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `penguin models status` (CLI).
...
```bash
curl -fsSL https://penguin.bot/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. Run `penguin channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Penguin scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/penguin/penguin/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.penguin.* still works)
```
...
Ensure the global `penguin` CLI is installed and matches the app version:

```bash
penguin --version
npm install -g penguin@<version>
```
...
#   ${PENGUIN_CONFIG_PATH:-$HOME/.penguin/penguin.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
penguin gateway --verbose
penguin channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/penguin/penguin-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$PENGUIN_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.penguin/logs/...`; profiles use `~/.penguin-<profile>/logs/...`)<br />Linux: `journalctl --user -u penguin-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Penguin Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$PENGUIN_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$PENGUIN_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$PENGUIN_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
penguin gateway status --deep

# Is the gateway reachable?
penguin health --json
# If it fails, rerun with connection details:
penguin health --verbose
...
tail -20 /tmp/penguin/penguin-*.log
```
...
trash "${PENGUIN_STATE_DIR:-$HOME/.penguin}"
penguin channels login         # re-pair WhatsApp
penguin gateway restart           # or: penguin gateway
```
...
1. Check logs first: `/tmp/penguin/` (default: `penguin-YYYY-MM-DD.log`, or your configured `logging.file`)
