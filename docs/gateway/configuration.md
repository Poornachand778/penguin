---
summary: "All configuration options for ~/.penguin/penguin.json with examples"
read_when:
...
Penguin reads an optional **JSON5** config from `~/.penguin/penguin.json` (comments + trailing commas allowed).

If the file is missing, Penguin uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.penguin/workspace`). You usually only need a config to:
...
Penguin only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `penguin doctor`, `penguin logs`, `penguin health`, `penguin status`, `penguin service`, `penguin help`).
- Run `penguin doctor` to see the exact issues.
- Run `penguin doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `penguin config set`. Keep a backup of `~/.penguin/penguin.json`.
...
```bash
penguin gateway call config.get --params '{}' # capture payload.hash
penguin gateway call config.apply --params '{
...
```bash
penguin gateway call config.get --params '{}' # capture payload.hash
penguin gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.penguin/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@penguin", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.penguin/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.penguin/workspace" }],
}
```
...
{ "$include": "/etc/penguin/base.json5" } // absolute
...
```json5
// ~/.penguin/clients/mueller/agents.json5
[
...
```json5
// ~/.penguin/clients/mueller/broadcast.json5
{
...
Penguin reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Penguin runs your login shell and imports only the missing expected keys (never overrides).
...
- `PENGUIN_LOAD_SHELL_ENV=1`
- `PENGUIN_SHELL_ENV_TIMEOUT_MS=15000`
...
Penguin stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.penguin/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.penguin/credentials/oauth.json` (or `$PENGUIN_STATE_DIR/credentials/oauth.json`)
...
- `~/.penguin/agent/*` (migrated by `penguin doctor` into `~/.penguin/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `PENGUIN_OAUTH_DIR`
- Agent dir (default agent root override): `PENGUIN_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Penguin imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Penguin derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/penguin/penguin-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/penguin/penguin.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.penguin/credentials/whatsapp/biz
          // authDir: "~/.penguin/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `penguin doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@penguin", "penguin"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@penguin"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.penguin/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.penguin/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.penguin/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.penguin/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.penguin/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.penguin/workspace-home" },
      { id: "work", workspace: "~/.penguin/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `penguin.json`).
...
Penguin starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["penguin-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-penguin",
...
Penguin starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `penguin plugins install @penguin/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Penguin starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Penguin spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Penguin
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/penguin" } },
}
```
...
When a file exceeds this limit, Penguin logs a warning and injects a truncated
...
message envelopes). If unset, Penguin uses the host timezone at runtime.
...
`[penguin]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[penguin]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[penguin]"`, Penguin will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Penguin generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.penguin/settings/tts.json",
      elevenlabs: {
...
Penguin also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Penguin currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `penguin configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:penguin`: all built-in Penguin tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.penguin/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.penguin/sandboxes",
        docker: {
          ...
          apparmorProfile: "penguin-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "penguin-sandbox-browser:bookworm-slim",
          containerPrefix: "penguin-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Penguin uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.penguin/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Penguin config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Penguin writes/merges a `models.json` into
`~/.penguin/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Penguin uses
...
- Shortcut: `penguin onboard --auth-choice opencode-zen`.
...
Shortcut: `penguin onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `penguin onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `penguin onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `penguin onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `penguin onboard --auth-choice minimax-api`.
...
- Override the agent config root with `PENGUIN_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.penguin/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.penguin/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.penguin/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Penguin stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.penguin/skills` (workspace skills
...
from `~/.penguin/extensions`, `<workspace>/.penguin/extensions`, plus any
...
### `browser` (penguin-managed browser)

Penguin can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for penguin and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Penguin.app menubar, or `penguin gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      penguin: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/penguin" }
...
- Examples: `"/ui"`, `"/penguin"`, `"/apps/penguin"`.
...
- When a connection comes from one of these IPs, Penguin uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `penguin gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `PENGUIN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `PENGUIN_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Penguin
...
- Penguin.app watches `~/.penguin/penguin.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.penguin/penguin.json` (or `PENGUIN_CONFIG_PATH`) and applies changes automatically.
...
- `~/.penguin/penguin.json` (or `PENGUIN_CONFIG_PATH`)
...
- `PENGUIN_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `penguin --dev …` → uses `~/.penguin-dev` + shifts ports from base `19001`
- `penguin --profile <name> …` → uses `~/.penguin-<name>` (port via config/env/flags)
...
```bash
PENGUIN_CONFIG_PATH=~/.penguin/a.json \
PENGUIN_STATE_DIR=~/.penguin-a \
penguin gateway --port 19001
```
...
Gmail helper config (used by `penguin webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "penguin@gmail.com",
...
- Set `PENGUIN_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Penguin defaults `serve.path` to `/` so
...
Default root: `~/.penguin/workspace/canvas`  
Default port: `18793` (chosen to avoid the penguin browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__penguin__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__penguin__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `PENGUIN_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.penguin/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.penguin/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.penguin/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_penguin-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `penguin` (advertises `penguin.local`). Override with `PENGUIN_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_penguin-gw._tcp` under `~/.penguin/dns/` using the configured discovery domain (example: `penguin.internal.`).
