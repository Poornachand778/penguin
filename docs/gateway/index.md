...
- Replaces the legacy `gateway` command. CLI entry point: `penguin gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
penguin gateway --port 18789
# for full debug/trace logs in stdio:
penguin gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
penguin gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.penguin/penguin.json` (or `PENGUIN_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__penguin__/canvas/` from `~/.penguin/workspace/canvas`. Disable with `canvasHost.enabled=false` or `PENGUIN_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `PENGUIN_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `PENGUIN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.penguin.*` may still exist)
- Linux: `penguin-gateway-<profile>.service`
- Windows: `Penguin Gateway (<profile>)`

Install metadata is embedded in the service config:

- `PENGUIN_SERVICE_MARKER=penguin`
- `PENGUIN_SERVICE_KIND=gateway`
- `PENGUIN_SERVICE_VERSION=<version>`
...
```bash
penguin --dev setup
penguin --dev gateway --allow-unconfigured
...
- `PENGUIN_STATE_DIR=~/.penguin-dev`
- `PENGUIN_CONFIG_PATH=~/.penguin-dev/penguin.json`
- `PENGUIN_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.penguin/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `PENGUIN_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `PENGUIN_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `PENGUIN_CONFIG_PATH`
- unique `PENGUIN_STATE_DIR`
...
```bash
penguin --profile main gateway install
penguin --profile rescue gateway install
```

Example:

```bash
PENGUIN_CONFIG_PATH=~/.penguin/a.json PENGUIN_STATE_DIR=~/.penguin-a penguin gateway --port 19001
PENGUIN_CONFIG_PATH=~/.penguin/b.json PENGUIN_STATE_DIR=~/.penguin-b penguin gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `penguin health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `penguin`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `penguin gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.penguin.*` is cleaned up).
  - `penguin doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Penguin profile services.
  ...
  - Cleanup: `penguin gateway uninstall` (current service) and `penguin doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `penguin gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Penguin.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.penguin.*` labels still unload cleanly).
- To stop it cleanly, use `penguin gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `penguin gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `penguin gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Penguin installs a **systemd user service** by default on Linux/WSL2. We
...
`penguin gateway install` writes the user unit. `penguin doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/penguin-gateway[-<profile>].service`:

```
[Unit]
Description=Penguin Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/penguin gateway --port 18789
Restart=always
RestartSec=5
Environment=PENGUIN_GATEWAY_TOKEN=
...
```
systemctl --user enable --now penguin-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/penguin-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now penguin-gateway[-<profile>].service
```
...
- `penguin gateway health|status` — request health/status over the Gateway WS.
- `penguin message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `penguin agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `penguin gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `penguin gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `penguin gateway` and the legacy TCP control port.
