...
Tlon is a decentralized messenger built on Urbit. Penguin connects to your Urbit ship and can
...
```bash
penguin plugins install @penguin/tlon
```

Local checkout (when running from a git repo):

```bash
penguin plugins install ./extensions/tlon
```
...
Use these with `penguin message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Penguin replies in-thread.
