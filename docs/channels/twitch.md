...
read_when:
  - Setting up Twitch chat integration for Penguin
---

# Twitch (plugin)

Twitch chat support via IRC connection. Penguin connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
penguin plugins install @penguin/twitch
```

Local checkout (when running from a git repo):

```bash
penguin plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `PENGUIN_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "penguin", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use PENGUIN_TWITCH_ACCESS_TOKEN env var)
...
```bash
PENGUIN_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
