...
3. Set tokens for Penguin and start the gateway.
...
7. Slash Commands → create `/penguin` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
Penguin can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Penguin falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Penguin",
    "description": "Slack connector for Penguin"
  },
  "features": {
    "bot_user": {
    ...
    "slash_commands": [
      {
        "command": "/penguin",
        "description": "Send a message to Penguin",
        "should_escape": false
      }
    ]
  },
...
By default, Penguin replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Penguin infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `penguin pairing approve slack <code>`.
...
- On startup, Penguin resolves channel/user names in allowlists to IDs (when tokens allow)
