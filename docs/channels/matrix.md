...
Matrix is an open, decentralized messaging protocol. Penguin connects as a Matrix **user**
...
```bash
penguin plugins install @penguin/matrix
```

Local checkout (when running from a git repo):

```bash
penguin plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `penguin plugins install @penguin/matrix`
   - From a local checkout: `penguin plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Penguin calls the same
     login endpoint, stores the access token in `~/.penguin/credentials/matrix/credentials.json`,
...
- On first connection, Penguin requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Penguin logs a warning.
...
`~/.penguin/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `penguin pairing list matrix`
  - `penguin pairing approve matrix <CODE>`
...
- On startup, Penguin resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
