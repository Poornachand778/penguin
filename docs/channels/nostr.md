...
read_when:
  - You want Penguin to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Penguin to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`penguin onboard`) and `penguin channels add` list optional channel plugins.
...
```bash
penguin plugins install @penguin/nostr
```

Use a local checkout (dev workflows):

```bash
penguin plugins install --link <path-to-penguin>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "penguin@example.com",
        "lud16": "penguin@example.com"
      }
    }
  }
}
