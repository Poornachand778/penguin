...
```bash
penguin plugins install @penguin/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
penguin plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Penguin" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `penguin pairing list nextcloud-talk`
  - `penguin pairing approve nextcloud-talk <CODE>`
