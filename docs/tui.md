...
```bash
penguin tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `penguin tui --deliver`
...
- Check the Gateway logs: `penguin logs --follow`.
- Confirm the agent can run: `penguin status` and `penguin models status`.
...
- No agents in picker: check `penguin agents list` and your routing config.
