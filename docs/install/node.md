...
read_when:
  - "You installed Penguin but `penguin` is “command not found”"
...
Penguin’s runtime baseline is **Node 22+**.

If you can run `npm install -g penguin@latest` but later see `penguin: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
