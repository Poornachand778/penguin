...
// NOTE: This extension is intended to be bundled with Penguin.
// When running from source (tests/dev), Penguin internals live under src/.
...
export function createLlmTaskTool(api: PenguinPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via penguin.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-llm-task-"));
