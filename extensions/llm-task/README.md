...
`penguin.invoke --each`) without adding new Penguin code per workflow.
...
This extension depends on Penguin internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Penguin extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.penguin/extensions` as a standalone plugin directory.
