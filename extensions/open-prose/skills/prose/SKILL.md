...
metadata: { "penguin": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Penguin `sessions_spawn`
- **File I/O** == Penguin `read`/`write`
- **Remote fetch** == Penguin `web_fetch` (or `exec` with curl when POST is required)
