...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as PenguinConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as PenguinConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as PenguinConfig);
