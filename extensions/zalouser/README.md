...
Penguin extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
penguin channels login --channel zalouser
...
```bash
penguin message send --channel zalouser --target <threadId> --message "Hello from Penguin!"
```
...
```bash
penguin channels login --channel zalouser              # Login via QR
penguin channels login --channel zalouser --account work
penguin channels status --probe
penguin channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
penguin directory self --channel zalouser
penguin directory peers list --channel zalouser --query "name"
penguin directory groups list --channel zalouser --query "work"
penguin directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
penguin message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
penguin message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `penguin channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
penguin channels login --channel zalouser --account work
penguin message send --channel zalouser --account work --target <id> --message "Hello"
