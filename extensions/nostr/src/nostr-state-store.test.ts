...
import type { PluginRuntime } from "penguin/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.PENGUIN_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-nostr-"));
  process.env.PENGUIN_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(homedir(), ".penguin");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.PENGUIN_STATE_DIR = previous;
    }
