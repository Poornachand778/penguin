...
import type { PenguinPluginApi, PenguinPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "penguin-lobster-plugin-") {
...
function fakeApi(): PenguinPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<PenguinPluginToolContext> = {}): PenguinPluginToolContext {
  return {
    config: {} as any,
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: PenguinPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
