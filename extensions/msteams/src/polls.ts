...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["penguinPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["penguin", "pollId"]) ??
    readNestedString(value, ["penguin", "poll", "id"]) ??
    readNestedString(value, ["data", "penguinPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "penguin", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          penguinPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "penguin poll vote",
          displayText: "Vote recorded",
          value: { penguinPollId: pollId, pollId },
        },
      },
    ],
  };
