...
function resolvePenguinRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.PENGUIN_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set PENGUIN_ROOT to the package root.");
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolvePenguinRoot();
