/**
 * Twitch channel plugin for Penguin.
 *
...
import { buildChannelConfigSchema } from "penguin/plugin-sdk";
 ...
 * for Penguin. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: PenguinConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: PenguinConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: PenguinConfig): boolean => {
