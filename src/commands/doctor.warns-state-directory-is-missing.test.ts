...
beforeEach(() => {
  ...
  resolvePenguinPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.PENGUIN_STATE_DIR;
  originalUpdateInProgress = process.env.PENGUIN_UPDATE_IN_PROGRESS;
  process.env.PENGUIN_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "penguin-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.PENGUIN_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.PENGUIN_UPDATE_IN_PROGRESS;
  } else {
    process.env.PENGUIN_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolvePenguinPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadPenguinPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/penguin-root.js", () => ({
  resolvePenguinPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "penguin-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.PENGUIN_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when PENGUIN_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/penguin.json",
      exists: true,
      raw: "{}",
    ...
    process.env.PENGUIN_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
