...
beforeAll(() => {
  previousProfile = process.env.PENGUIN_PROFILE;
  process.env.PENGUIN_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.PENGUIN_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/penguin-root.js", () => ({
  resolvePenguinPackageRoot: vi.fn().mockResolvedValue("/tmp/penguin"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/penguin/pnpm-lock.yaml",
      markerPath: "/tmp/penguin/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Penguin status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("penguin status --all") ||
          l.includes("penguin --profile isolated status --all") ||
          l.includes("penguin status --all") ||
          l.includes("penguin --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.PENGUIN_GATEWAY_TOKEN = "abcd1234";
    try {
