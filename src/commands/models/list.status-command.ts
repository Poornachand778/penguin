...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolvePenguinAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("penguin models auth setup-token")}\` or \`${formatCliCommand("penguin configure")}\`.`
          : `Run \`${formatCliCommand("penguin configure")}\` or set an API key env var.`;
