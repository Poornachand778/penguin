...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolvePenguinAgentDir: vi.fn().mockReturnValue("/tmp/penguin-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/penguin-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/penguin-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolvePenguinAgentDir: mocks.resolvePenguinAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolvePenguinAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/penguin-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/penguin-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/penguin-agent-custom");
