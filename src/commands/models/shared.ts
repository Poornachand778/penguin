...
export async function updateConfig(
  mutator: (cfg: PenguinConfig) => PenguinConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: PenguinConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: PenguinConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("penguin agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
