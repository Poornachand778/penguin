...
const requireAgentDir = () => {
  const agentDir = process.env.PENGUIN_AGENT_DIR;
  if (!agentDir) {
    throw new Error("PENGUIN_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.PENGUIN_STATE_DIR;
  const previousAgentDir = process.env.PENGUIN_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.PENGUIN_AGENT_DIR;
    } else {
      process.env.PENGUIN_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.PENGUIN_OAUTH_DIR;
  });

  it("writes auth-profiles.json under PENGUIN_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-oauth-"));
    process.env.PENGUIN_STATE_DIR = tempStateDir;
    process.env.PENGUIN_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.PENGUIN_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to PENGUIN_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-minimax-"));
    process.env.PENGUIN_STATE_DIR = tempStateDir;
    process.env.PENGUIN_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.PENGUIN_AGENT_DIR;
