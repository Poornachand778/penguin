...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Penguin status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("penguin security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("penguin security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.penguin.ai/faq");
  runtime.log("Troubleshooting: https://docs.penguin.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("penguin status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("penguin logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("penguin status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("penguin gateway probe")}`);
  }
}
