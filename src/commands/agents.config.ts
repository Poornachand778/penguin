...
export function listAgentEntries(cfg: PenguinConfig): AgentEntry[] {
...
function resolveAgentName(cfg: PenguinConfig, agentId: string) {
...
function resolveAgentModel(cfg: PenguinConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: PenguinConfig): AgentSummary[] {
