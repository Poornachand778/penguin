...
const SEARCH_TOOL = "https://docs.penguin.ai/mcp.SearchPenguin";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.penguin.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('penguin docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.penguin.ai/");
      runtime.log(`Search: ${formatCliCommand('penguin docs "your query"')}`);
    }
    return;
  }
