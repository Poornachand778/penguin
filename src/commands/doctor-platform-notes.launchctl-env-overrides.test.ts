...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "PENGUIN_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("PENGUIN_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv PENGUIN_GATEWAY_TOKEN");
    expect(message).not.toContain("PENGUIN_GATEWAY_PASSWORD");
  });
