...
function resolveMode(cfg: PenguinConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Penguin doctor");

  const root = await resolvePenguinPackageRoot({
  ...
  let cfg: PenguinConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("penguin configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("penguin config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("penguin setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("penguin doctor --fix")}" to apply changes.`);
  }
