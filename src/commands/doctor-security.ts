...
import type { PenguinConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: PenguinConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("penguin security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("penguin configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("penguin config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("penguin doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("penguin config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
