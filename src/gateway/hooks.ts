...
export function resolveHooksConfig(cfg: PenguinConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-penguin-token"] === "string"
      ? req.headers["x-penguin-token"].trim()
      : "";
  if (headerToken) {
