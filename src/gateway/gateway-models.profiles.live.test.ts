...
import { resolvePenguinAgentDir } from "../agents/agent-paths.js";
...
import { ensurePenguinModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { PenguinConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.PENGUIN_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.PENGUIN_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.PENGUIN_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<PenguinConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.PENGUIN_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.PENGUIN_STATE_DIR,
  };
  ...
  const hostAgentDir = resolvePenguinAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-live-state-"));
  process.env.PENGUIN_STATE_DIR = tempStateDir;
  ...
  process.env.PENGUIN_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.penguin-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolvePenguinAgentDir();
  const sanitizedCfg: PenguinConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Penguin live tool probe (local, safe): " +
    ...
    process.env.PENGUIN_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.PENGUIN_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensurePenguinModelsJson(cfg);
      ...
      const rawModels = process.env.PENGUIN_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.penguin-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
