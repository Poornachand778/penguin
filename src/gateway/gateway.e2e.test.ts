...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.PENGUIN_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-gw-mock-home-"));
      ...
      process.env.PENGUIN_SKIP_BROWSER_CONTROL_SERVER = "1";

      const token = `test-${randomUUID()}`;
      process.env.PENGUIN_GATEWAY_TOKEN = token;

      const workspaceDir = path.join(tempHome, "penguin");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.penguin-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".penguin");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "penguin.json");
      ...
      process.env.PENGUIN_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.PENGUIN_CONFIG_PATH = prev.configPath;
        process.env.PENGUIN_GATEWAY_TOKEN = prev.token;
        process.env.PENGUIN_SKIP_CHANNELS = prev.skipChannels;
        process.env.PENGUIN_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.PENGUIN_SKIP_CRON = prev.skipCron;
        process.env.PENGUIN_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.PENGUIN_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.PENGUIN_STATE_DIR = prev.stateDir;
      process.env.PENGUIN_CONFIG_PATH = prev.configPath;
      process.env.PENGUIN_GATEWAY_TOKEN = prev.token;
      process.env.PENGUIN_SKIP_CHANNELS = prev.skipChannels;
      process.env.PENGUIN_SKIP_GMAIL_WATCHER = prev.skipGmail;
