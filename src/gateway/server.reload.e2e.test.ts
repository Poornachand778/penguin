...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.PENGUIN_SKIP_CHANNELS;
    prevSkipGmail = process.env.PENGUIN_SKIP_GMAIL_WATCHER;
    process.env.PENGUIN_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.PENGUIN_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.PENGUIN_SKIP_GMAIL_WATCHER;
    } else {
      process.env.PENGUIN_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
