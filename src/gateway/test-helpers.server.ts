...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.PENGUIN_STATE_DIR;
  previousConfigPath = process.env.PENGUIN_CONFIG_PATH;
  previousSkipBrowserControl = process.env.PENGUIN_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.PENGUIN_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.PENGUIN_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.PENGUIN_STATE_DIR = path.join(tempHome, ".penguin");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "penguin-test-"))
    : path.join(tempHome, ".penguin-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.PENGUIN_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.PENGUIN_CONFIG_PATH;
    } else {
      process.env.PENGUIN_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.PENGUIN_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.PENGUIN_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.PENGUIN_SKIP_GMAIL_WATCHER;
    } else {
      process.env.PENGUIN_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.PENGUIN_SKIP_CANVAS_HOST;
    } else {
      process.env.PENGUIN_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.PENGUIN_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.PENGUIN_GATEWAY_TOKEN = fallbackToken;
  }
