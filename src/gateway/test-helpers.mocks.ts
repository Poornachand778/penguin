...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `penguin-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.PENGUIN_CONFIG_PATH = path.join(root, "penguin.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "penguin.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "penguin-gateway-test"),
        ...fileDefaults,
