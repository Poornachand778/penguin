...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("penguinCanvasA2UIAction");
    expect(out).toContain("penguinSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("penguinSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.penguinA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__penguin__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("penguin-a2ui-host");
      expect(html).toContain("penguinCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__penguin__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("penguinA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
