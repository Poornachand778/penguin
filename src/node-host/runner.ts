...
const execHostEnforced = process.env.PENGUIN_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.PENGUIN_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensurePenguinCliOnPath({ pathEnv: process.env.PATH ?? "" });
