...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.PENGUIN_GATEWAY_PORT).toBe("18789");
    expect(env.PENGUIN_GATEWAY_TOKEN).toBe("secret");
    expect(env.PENGUIN_SERVICE_MARKER).toBe("penguin");
    expect(env.PENGUIN_SERVICE_KIND).toBe("gateway");
    expect(typeof env.PENGUIN_SERVICE_VERSION).toBe("string");
    expect(env.PENGUIN_SYSTEMD_UNIT).toBe("penguin-gateway.service");
    if (process.platform === "darwin") {
      expect(env.PENGUIN_LAUNCHD_LABEL).toBe("ai.penguin.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", PENGUIN_PROFILE: "work" },
      port: 18789,
    });
    expect(env.PENGUIN_SYSTEMD_UNIT).toBe("penguin-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.PENGUIN_LAUNCHD_LABEL).toBe("ai.penguin.work");
    }
  });
});
