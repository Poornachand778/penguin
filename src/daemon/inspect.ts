...
export function renderGatewayServiceCleanupHints(
  ...
  const profile = env.PENGUIN_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["penguin_service_marker"];
  const kindKeys = ["penguin_service_kind"];
...
function isPenguinGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.penguin.");
}

function isPenguinGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("penguin-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isPenguinGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("penguin gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("penguin") || lower.includes("penguin");
}
  ...
  for (const entry of entries) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "penguin" : "penguin",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "penguin" && isPenguinGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "penguin" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "penguin" && isPenguinGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isPenguinGatewayTaskName(name)) {
        continue;
      }
