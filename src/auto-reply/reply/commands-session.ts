...
import { scheduleGatewaySigusr1Restart, triggerPenguinRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Penguin in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerPenguinRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Penguin via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
