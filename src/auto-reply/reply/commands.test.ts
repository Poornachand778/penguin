...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-commands-"));
...
function buildParams(commandBody: string, cfg: PenguinConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
