...
type AgentConfig = Partial<NonNullable<NonNullable<PenguinConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.penguin/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 Penguin ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: PenguinConfig): string {
  const lines = ["ℹ️ Help", ""];
