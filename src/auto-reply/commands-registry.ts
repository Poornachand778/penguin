import type { PenguinConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: PenguinConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: PenguinConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as PenguinConfig),
...
export function getCommandDetection(_cfg?: PenguinConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: PenguinConfig) {
