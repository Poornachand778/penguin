...
import { discoverPenguinPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: PenguinPluginDefinition;
  register?: PenguinPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as PenguinPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as PenguinPluginDefinition;
...
export function loadPenguinPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverPenguinPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "penguin/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: PenguinPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as PenguinPluginModule;
    } catch (err) {
