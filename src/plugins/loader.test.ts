...
import { loadPenguinPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.PENGUIN_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `penguin-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "penguin.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.PENGUIN_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadPenguinPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.PENGUIN_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadPenguinPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadPenguinPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
