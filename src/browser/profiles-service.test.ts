...
vi.mock("./chrome.js", () => ({
  resolvePenguinUserDataDir: vi.fn(() => "/tmp/penguin-test/penguin/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolvePenguinUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "penguin-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolvePenguinUserDataDir).mockReturnValue(userDataDir);
