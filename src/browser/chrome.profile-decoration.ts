...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".penguin-profile-decorated");
}
 ...
 */
export function decoratePenguinProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_PENGUIN_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_PENGUIN_BROWSER_COLOR).toUpperCase();
