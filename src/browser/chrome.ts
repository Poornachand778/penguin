...
export function resolvePenguinUserDataDir(profileName = DEFAULT_PENGUIN_BROWSER_PROFILE_NAME) {
...
export async function launchPenguinChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_PENGUIN_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decoratePenguinProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`penguin browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopPenguinChrome(running: RunningChrome, timeoutMs = 2500) {
