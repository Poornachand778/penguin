import type { BrowserConfig, BrowserProfileConfig, PenguinConfig } from "../config/config.js";
import {
...
import {
  DEFAULT_PENGUIN_BROWSER_COLOR,
  DEFAULT_PENGUIN_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_PENGUIN_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_PENGUIN_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "penguin" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_PENGUIN_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_PENGUIN_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Penguin browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for penguin/penguin CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: PenguinConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_PENGUIN_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "penguin";
