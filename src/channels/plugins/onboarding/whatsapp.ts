...
function setWhatsAppDmPolicy(cfg: PenguinConfig, dmPolicy: DmPolicy): PenguinConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: PenguinConfig, allowFrom?: string[]): PenguinConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: PenguinConfig, selfChatMode: boolean): PenguinConfig {
...
async function detectWhatsAppLinked(cfg: PenguinConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Penguin can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Penguin" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("penguin channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
