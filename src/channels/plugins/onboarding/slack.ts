...
function setSlackDmPolicy(cfg: PenguinConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Penguin";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Penguin`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: PenguinConfig, allowFrom: string[]): PenguinConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
