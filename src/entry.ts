...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.PENGUIN_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.PENGUIN_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.PENGUIN_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[penguin] ${parsed.error}`);
    process.exit(2);
  }
