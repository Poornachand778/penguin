...
export function triggerPenguinRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.PENGUIN_SYSTEMD_UNIT,
        process.env.PENGUIN_PROFILE,
      );
  ...
  const label =
    process.env.PENGUIN_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.PENGUIN_PROFILE);
