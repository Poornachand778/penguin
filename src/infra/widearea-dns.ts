...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.PENGUIN_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*penguin-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "penguin";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "penguin");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "penguin-gw");
  ...
  records.push(`_penguin-gw._tcp IN PTR ${instanceLabel}._penguin-gw._tcp`);
  records.push(`${instanceLabel}._penguin-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._penguin-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; penguin-content-hash: ${contentHash}\n${contentBody}`;
}
