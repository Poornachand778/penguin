...
function candidateBinDirs(opts: EnsurePenguinPathOpts): string[] {
  ...
  // Bundled macOS app: `penguin` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "penguin");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/penguin` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "penguin"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `penguin` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensurePenguinCliOnPath(opts: EnsurePenguinPathOpts = {}) {
  if (isTruthyEnvValue(process.env.PENGUIN_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.PENGUIN_PATH_BOOTSTRAPPED = "1";
