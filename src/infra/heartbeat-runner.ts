...
function resolveActiveHoursTimezone(cfg: PenguinConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: PenguinConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: PenguinConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: PenguinConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: PenguinConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: PenguinConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: PenguinConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: PenguinConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
