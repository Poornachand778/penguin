...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.PENGUIN_MDNS_HOSTNAME = "test-host";
    process.env.PENGUIN_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("penguin-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("penguin (Penguin)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("penguin");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("penguin.local");
