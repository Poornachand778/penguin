...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "penguin doctor",
        managerScriptArgs(manager, "penguin", ["doctor", "--non-interactive"]),
        gitRoot,
        { PENGUIN_UPDATE_IN_PROGRESS: "1" },
      ),
    );
