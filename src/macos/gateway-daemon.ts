...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `penguin --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.PENGUIN_GATEWAY_PORT ??
    process.env.PENGUIN_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.PENGUIN_GATEWAY_BIND ??
    process.env.PENGUIN_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
