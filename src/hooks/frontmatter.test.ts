...
`;
    ...
    expect(parsed.penguin.emoji).toBe("💾");
    expect(parsed.penguin.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.penguin.emoji).toBe("📝");
    expect(parsed.penguin.events).toEqual(["command"]);
    expect(parsed.penguin.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.penguin.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"penguin": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"penguin": {"events": ["test"]}}');
  });
...
describe("resolvePenguinMetadata", () => {
  it("extracts penguin metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolvePenguinMetadata(frontmatter);
  ...
  it("returns undefined when penguin key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        penguin: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Penguin" },
            { id: "npm", kind: "npm", package: "@penguin/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@penguin/hook");
  });
    ...
    "penguin":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Penguin" }],
      },
  }
---
...
`;
    ...
    const penguin = resolvePenguinMetadata(frontmatter);
    expect(penguin).toBeDefined();
    expect(penguin?.emoji).toBe("💾");
    expect(penguin?.events).toEqual(["command:new"]);
    expect(penguin?.requires?.config).toEqual(["workspace.dir"]);
    expect(penguin?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const penguin = resolvePenguinMetadata(frontmatter);
    expect(penguin?.emoji).toBe("disk");
    expect(penguin?.events).toEqual(["command:new"]);
  });
});
