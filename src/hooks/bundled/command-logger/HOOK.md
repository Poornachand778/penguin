...
homepage: https://docs.penguin.ai/hooks#command-logger
metadata:
  {
    "penguin":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.penguin/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.penguin/logs/commands.log ~/.penguin/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/penguin`:
   ```
   /home/username/.penguin/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
