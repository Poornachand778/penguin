...
This directory contains hooks that ship with Penguin. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.penguin.ai/hooks/soul-evil
...
Hooks can be configured in `~/.penguin/penguin.json`:
...
2. Restart gateway: `pkill -9 -f 'penguin.*gateway' && pnpm penguin gateway`
3. Enable the hook: `penguin hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.penguin.ai/hooks
