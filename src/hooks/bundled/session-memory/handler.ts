...
const saveSessionToMemory: HookHandler = async (event) => {
  ...
  try {
    ...
    const cfg = context.cfg as PenguinConfig | undefined;
    const agentId = resolveAgentIdFromSessionKey(event.sessionKey);
    const workspaceDir = cfg
      ? resolveAgentWorkspaceDir(cfg, agentId)
      : path.join(os.homedir(), ".penguin", "workspace");
    ...
    if (sessionFile) {
      ...
      if (sessionContent && cfg) {
        ...
        const penguinRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
        const slugGenPath = path.join(penguinRoot, "llm-slug-generator.js");
