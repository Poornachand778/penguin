...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `penguin-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@penguin/zip-hooks",
        version: "0.0.1",
        penguin: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@penguin/tar-hooks",
        version: "0.0.1",
        penguin: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
