...
import type { PenguinConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: PenguinConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: PenguinConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep PENGUIN_STATE_DIR on a local-only volume and re-run "${formatCliCommand("penguin security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: PenguinConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer PENGUIN_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: PenguinConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: PenguinConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: PenguinConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: PenguinConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: PenguinConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: PenguinConfig): boolean {
...
function isBrowserEnabled(cfg: PenguinConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: PenguinConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectExposureMatrixFindings(cfg: PenguinConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
