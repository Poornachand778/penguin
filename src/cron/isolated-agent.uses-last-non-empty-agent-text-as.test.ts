...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "penguin-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".penguin", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<PenguinConfig> = {},
): PenguinConfig {
  const base: PenguinConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "penguin"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".penguin", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
