...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("penguin node install"), formatCliCommand("penguin node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("penguin node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    PENGUIN_LOG_PREFIX: baseEnv.PENGUIN_LOG_PREFIX ?? "node",
