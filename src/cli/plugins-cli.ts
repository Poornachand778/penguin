...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: PenguinConfig; warnings: string[] } {
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage Penguin plugins/extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.penguin.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.penguin.ai/plugin");
      lines.push("");
