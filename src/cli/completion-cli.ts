...
export async function installCompletion(shell: string, yes: boolean, binName = "penguin") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c penguin -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'penguin gateway' and not 'penguin other gateway', we need to check the sequence.
