...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "penguin-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      PENGUIN_NO_RESPAWN: "1",
      PENGUIN_STATE_DIR: stateDir,
      PENGUIN_SKIP_CHANNELS: "1",
      PENGUIN_SKIP_GMAIL_WATCHER: "1",
      PENGUIN_SKIP_CRON: "1",
      PENGUIN_SKIP_BROWSER_CONTROL_SERVER: "1",
      PENGUIN_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "penguin-entry-bootstrap.mjs");
