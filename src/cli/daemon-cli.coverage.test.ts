...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    PENGUIN_STATE_DIR: process.env.PENGUIN_STATE_DIR,
    PENGUIN_CONFIG_PATH: process.env.PENGUIN_CONFIG_PATH,
    PENGUIN_GATEWAY_PORT: process.env.PENGUIN_GATEWAY_PORT,
    PENGUIN_PROFILE: process.env.PENGUIN_PROFILE,
  };

  beforeEach(() => {
    process.env.PENGUIN_STATE_DIR = "/tmp/penguin-cli-state";
    process.env.PENGUIN_CONFIG_PATH = "/tmp/penguin-cli-state/penguin.json";
    delete process.env.PENGUIN_GATEWAY_PORT;
    delete process.env.PENGUIN_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.PENGUIN_STATE_DIR !== undefined) {
      process.env.PENGUIN_STATE_DIR = originalEnv.PENGUIN_STATE_DIR;
    } else {
      delete process.env.PENGUIN_STATE_DIR;
    }

    if (originalEnv.PENGUIN_CONFIG_PATH !== undefined) {
      process.env.PENGUIN_CONFIG_PATH = originalEnv.PENGUIN_CONFIG_PATH;
    } else {
      delete process.env.PENGUIN_CONFIG_PATH;
    }

    if (originalEnv.PENGUIN_GATEWAY_PORT !== undefined) {
      process.env.PENGUIN_GATEWAY_PORT = originalEnv.PENGUIN_GATEWAY_PORT;
    } else {
      delete process.env.PENGUIN_GATEWAY_PORT;
    }

    if (originalEnv.PENGUIN_PROFILE !== undefined) {
      process.env.PENGUIN_PROFILE = originalEnv.PENGUIN_PROFILE;
    } else {
      delete process.env.PENGUIN_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        PENGUIN_PROFILE: "dev",
        PENGUIN_STATE_DIR: "/tmp/penguin-daemon-state",
        PENGUIN_CONFIG_PATH: "/tmp/penguin-daemon-state/penguin.json",
        PENGUIN_GATEWAY_PORT: "19001",
      },
