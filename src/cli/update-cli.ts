...
import { resolvePenguinPackageRoot } from "../infra/penguin-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "penguin doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "penguin";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const PENGUIN_REPO_URL = "https://github.com/penguin/penguin.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".penguin");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("penguin@")) {
    return trimmed.slice("penguin@".length);
  }
...
function resolveGitInstallDir(): string {
  const override = process.env.PENGUIN_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", PENGUIN_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `PENGUIN_GIT_DIR points at a non-git directory: ${params.dir}. Set PENGUIN_GIT_DIR to an empty folder or an penguin checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`PENGUIN_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolvePenguinPackageRoot({
  ...
  defaultRuntime.log(theme.heading("Penguin update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Penguin..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Penguin install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("penguin doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("penguin gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g penguin@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g penguin@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.PENGUIN_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.PENGUIN_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("penguin gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("penguin doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("penguin gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("penguin gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `penguin update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `PENGUIN_GIT_DIR points at a non-git directory: ${gitDir}. Set PENGUIN_GIT_DIR to an empty folder or an penguin checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via PENGUIN_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Penguin to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["penguin update", "Update a source checkout (git)"],
        ["penguin update --channel beta", "Switch to beta channel (git + npm)"],
        ["penguin update --channel dev", "Switch to dev channel (git + npm)"],
        ["penguin update --tag beta", "One-off update to a dist-tag or version"],
        ["penguin update --no-restart", "Update without restarting the service"],
        ["penguin update --json", "Output result as JSON"],
        ["penguin update --yes", "Non-interactive (accept downgrade prompts)"],
        ["penguin update wizard", "Interactive update wizard"],
        ["penguin --update", "Shorthand for penguin update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run penguin update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.penguin.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["penguin update status", "Show channel + version status."],
          ["penguin update status --json", "JSON output."],
          ["penguin update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.penguin.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
