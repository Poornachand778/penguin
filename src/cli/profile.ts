...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.penguin${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.PENGUIN_PROFILE = profile;

  const stateDir = env.PENGUIN_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.PENGUIN_STATE_DIR?.trim()) {
    env.PENGUIN_STATE_DIR = stateDir;
  }

  if (!env.PENGUIN_CONFIG_PATH?.trim()) {
    env.PENGUIN_CONFIG_PATH = path.join(stateDir, "penguin.json");
  }

  if (profile === "dev" && !env.PENGUIN_GATEWAY_PORT?.trim()) {
    env.PENGUIN_GATEWAY_PORT = "19001";
  }
}
