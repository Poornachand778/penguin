...
${formatHelpExamples([
  ['penguin agent --to +15555550123 --message "status update"', "Start a new session."],
  ['penguin agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'penguin agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'penguin agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['penguin agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'penguin agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.penguin.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.penguin.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "penguin agents set-identity --identity-file ~/.penguin/workspace/IDENTITY.md --agent main",
