...
import { PenguinSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof PenguinSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when Penguin writes the config.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /penguin).",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $PENGUIN_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.penguin/memory/{agentId}.sqlite).",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `penguin plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.penguin/extensions/<id>).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/penguin",
  "channels.mattermost.baseUrl": "https://chat.example.com",
  "agents.list[].identity.avatar": "avatars/penguin.png",
};
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = PenguinSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "PenguinConfig";
