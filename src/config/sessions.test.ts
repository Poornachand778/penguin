...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-penguin",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-penguin#general");
  });
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-sessions-"));
  ...
  it("derives session transcripts dir from PENGUIN_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { PENGUIN_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.PENGUIN_STATE_DIR = "/custom/state";
    try {
