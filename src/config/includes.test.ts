...
const ETC_PENGUIN_DIR = path.join(ROOT_DIR, "etc", "penguin");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "penguin.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcPenguinPath(...parts: string[]) {
  return path.join(ETC_PENGUIN_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcPenguinPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "penguin.json"))).toEqual({
      shared: true,
    });
  });
});
