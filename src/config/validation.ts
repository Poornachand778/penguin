...
import type { PenguinConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: PenguinConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: PenguinConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = PenguinSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as PenguinConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as PenguinConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as PenguinConfig)),
    ),
  };
}
