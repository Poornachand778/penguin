...
import type { PenguinConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { comparePenguinVersions } from "./version.js";
...
function coerceConfig(value: unknown): PenguinConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as PenguinConfig;
}
...
function stampConfigVersion(cfg: PenguinConfig): PenguinConfig {
...
function warnIfConfigFromFuture(cfg: PenguinConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = comparePenguinVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Penguin (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: PenguinConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): PenguinConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as PenguinConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as PenguinConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: PenguinConfig) {
    clearConfigCache();
...
// module scope. `PENGUIN_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.PENGUIN_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.PENGUIN_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): PenguinConfig {
...
export async function writeConfigFile(cfg: PenguinConfig): Promise<void> {
  clearConfigCache();
