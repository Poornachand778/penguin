...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "penguin.sandbox=1");
  args.push("--label", `penguin.sessionKey=${params.scopeKey}`);
  args.push("--label", `penguin.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `penguin.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("penguin.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`penguin sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`penguin sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("penguin sandbox recreate --all");
}
