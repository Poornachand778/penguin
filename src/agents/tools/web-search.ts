...
type WebSearchConfig = NonNullable<PenguinConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: PenguinConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("penguin configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.penguin.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://penguin.ai",
      "X-Title": "Penguin Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
