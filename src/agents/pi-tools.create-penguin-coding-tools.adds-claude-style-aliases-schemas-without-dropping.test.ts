...
import { __testing, createPenguinCodingTools } from "./pi-tools.js";
...
describe("createPenguinCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createPenguinTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createPenguinCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "penguin-outside.txt");
