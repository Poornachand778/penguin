...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-embedded-agent-"));
...
const ensureModels = (cfg: PenguinConfig) => ensurePenguinModelsJson(cfg, agentDir) as unknown;
