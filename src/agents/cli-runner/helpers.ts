...
/**
 * Cleanup suspended Penguin CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: PenguinConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-cli-images-"));
  const paths: string[] = [];
