...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "PENGUIN_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "penguin-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.PENGUIN_STATE_DIR = path.join(home, ".penguin");
    ...
    await fs.mkdir(path.join(home, ".penguin"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.penguin${path.sep}media`);
