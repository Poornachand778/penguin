...
describe("hasAnyWhatsAppAuth", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.PENGUIN_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "penguin-oauth-"));
    process.env.PENGUIN_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.PENGUIN_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("includes authDir overrides", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "penguin-wa-auth-"));
    try {
